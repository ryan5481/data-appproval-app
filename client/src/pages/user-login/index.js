  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (userType === 'user') {
        ...
        endpoint = `http://localhost:8000/initiator-login`;
      }
      ...
      if (response.status === 200) {
        ...
        toast(`Logged into ${userType} account.`,{ position: 'top-center', theme: 'light' });
        if(response.data.userRole == "user"){
          setTimeout(() => {router.push('/user-dashboard')}, 1000)
        }if(response.data.userRole == "initiator"){
          setTimeout(() => {router.push('/initiator-dashboard')}, 1000)
        }
      }
    } catch (error) {
  ...
  return (
    <main className="flex min-h-screen justify-center items-center p-24 w-full bg-blue-300">
      ...
      <div className='flex flex-col w-80 items-center justify-center bg-white p-5 rounded-lg shadow-lg'>
        ...
        {/* TITLE */}
        <div className='font-bold text-blue-500 text-[20px] p-1' >User Login</div>
        <form
